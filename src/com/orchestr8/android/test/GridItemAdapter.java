...
public class GridItemAdapter extends BaseAdapter {
	...
	public GridItemAdapter(Context c, ArrayList<String> concepts, String emotion) {
		...
		System.out
				.println("FUCK SEARCH URL "
						+ "https://www.google.com/search?start=1&q="
						+ grid_pictures.get(0).replace(" ", "%20")
						+ "&tbs=isz:lt,is&gbv=2&biw=1279&bih=679&sei=8ujrUfjiGOmTiQeqmIGQCg&tbm=isch#q="
						+ grid_pictures.get(0)
						+ "&safe=off&gbv=2&tbs=isz:i&tbm=isch&source=lnt&sa=X&ei=8ujrUcb6Ja-aiAeB8YGABQ&ved=0CBwQpwUoAzgB&bav=on.2,or.r_cp.r_qf.&bvm=bv.49478099%2Cd.aGc%2Cpv.xjs.s.en_US.c75bKy5EQ0A.O&fp=f895aedf61b43810&biw=1279&bih=679");
		System.out.println("FUCK SEARCH URL 2 "
				+ "http://images.google.com/images?gbv=1&start=" + 20 + "&q="
				+ grid_pictures.get(0).replace(" ", "%20")
				+ "&tbs=isz:ex,iszw:150,iszh:150");

		new RequestTask()
				.execute("http://images.google.com/images?gbv=1&start=" + 20
						+ "&q=" + grid_pictures.get(0).replace(" ", "%20")
						+ "&tbs=isz:ex,iszw:150,iszh:150");
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		grid_image = (SmartImageView) convertView.findViewById(R.id.imageView1);
	...
	class RequestTask extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... uri) {
			...
			// responseString =
			// responseString.substring(responseString.indexOf("imgurl=http"),
			// responseString.indexOf("imgurl=http") +
			// responseString.substring(responseString.indexOf("imgurl=http")).indexOf(".jpg"));
			...
			while (test.indexOf("imgurl") != -1) {
				test = test + " ";
				int index = test.indexOf("imgurl");
				System.out.println("INDEX jpg" + index + " KAY ");
				test = test.substring(index);
			}
			Pattern pattern = Pattern.compile("\"http://[\\S]{0,3}gstatic.com/(\\S+)\"");
			...
			while (matcher.find()) {
				...
				System.out.print("FUCK GOOGLE End index: " + matcher.end()
						+ " ");
				System.out.println(responseString.substring(matcher.start()+1,matcher.end()-1));
				String match =responseString.substring(matcher.start()+1,matcher.end()-1);
				return match;
		...
		protected void onPostExecute(String result) {
			...
			// String farm_id = result.substring(result.indexOf("farm") + 6,
			// result.indexOf(", \"title"));
			// String server_id = result.substring(result.indexOf("server") + 9,
			// result.indexOf(", \"farm")-1);
			// String id = result.substring(result.indexOf("id") + 5,
			// result.indexOf(", \"owner")-1);
			// String secret = result.substring(result.indexOf("secret") + 9,
			// result.indexOf(", \"server")-1);
			image_url = result;
			...
			if (counter == grid_pictures.size()) {
				...
				new RequestTask()
						.execute("http://images.google.com/images?gbv=1&start="
								+ 20
								+ "&q="
								+ grid_pictures.get(counter)
										.replace(" ", "%20")
								+ "&tbs=isz:ex,iszw:150,iszh:150");
